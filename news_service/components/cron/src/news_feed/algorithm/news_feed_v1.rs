...
use crate::openai::api::fetch_news_post_url_climate_classification;
use anyhow::Result;
use chrono::Local;
use db::constants::NEWS_FEED_URLS_NUM_DAYS;
use db::models::news_feed_url::NewsFeedUrl;
use db::sql::news_bsky_post_url::find_news_bsky_post_url_by_id;
...
pub async fn populate_news_feed_v1(db_pool: &PgPool) -> Result<()> {
    ...
    let author_score_map: HashMap<String, i32> = populate_author_score_map(&news_referenced_urls);
...
async fn populate_news_feed_urls_v1(
    db_pool: &PgPool,
    author_score_map: HashMap<String, i32>,
    ...
    for url_id in url_to_tweet_map.keys() {
        ...
        // Find the first post that shared the url
        ...
        if news_feed_url_db.is_none() && time_decayed_url_score > 0 {
            let news_feed_url = NewsFeedUrl {
                ...
                num_references,
                first_referenced_by: first_tweet.author_id.clone(),
...
async fn create_news_feed_url_slug_str(db_pool: &PgPool, url_id: i32) -> Result<String> {
    let news_bsky_post_url = find_news_bsky_post_url_by_id(db_pool, url_id).await?;
    let slugified_title = slugify(&news_bsky_post_url.title.unwrap_or_default());
    let start_num = 0u32;
...
async fn update_is_climate_related_fields(db_pool: &PgPool) -> Result<()> {
        ...
        find_top_news_feed_urls_without_is_climate_related_set(db_pool).await
    {
        for news_feed_url in news_feed_urls {
            let news_bsky_post_url =
                find_news_bsky_post_url_by_id(db_pool, news_feed_url.url_id).await?;

            match fetch_news_post_url_climate_classification(news_bsky_post_url).await {
                ...
                Err(err) => {
                    error!(
                        "fetch_news_post_url_climate_classification failed: {:?}",
                        err
                    );
                }
            }
