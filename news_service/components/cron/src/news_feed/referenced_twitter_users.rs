...
// pub async fn get_referenced_twitter_users(
//     db_pool: &PgPool,
//     twitter_api: &TwitterApi<BearerToken>,
// ) -> Result<()> {
//     let news_user_referenced_tweets = get_all_news_user_referenced_tweet_query(db_pool).await?;
//     info!(
...
//             if let Some(tweet_hashset) = author_map.get(&author_id) {
...
//         if let Some(tweet_hashset) = author_map.get(author_id) {
//             let tweet_hashset_len = tweet_hashset.len();
